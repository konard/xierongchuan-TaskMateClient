...
export const UserDetailsModal: React.FC<UserDetailsModalProps> = ({
 ...
 {[
 { label:'Вовремя', value: stats.completed_on_time, icon: CheckCircleIcon, color:'text-green-600', bg:'bg-green-50' },
 { label:'С опозданием', value: stats.completed_late, icon: ClockIcon, color:'text-yellow-600', bg:'bg-yellow-50' },
 { label:'Просрочено', value: stats.overdue_tasks, icon: XCircleIcon, color:'text-red-600', bg:'bg-red-50' },
 { label:'На проверке', value: stats.pending_review, icon: EyeIcon, color:'text-orange-600', bg:'bg-orange-50' },
 { label:'Отклонено', value: stats.rejected_tasks, icon: NoSymbolIcon, color:'text-purple-600', bg:'bg-purple-50' },
 { label:'Ср. время', value: formatTime(stats.avg_completion_time_hours), icon: ClockIcon, color:'text-blue-600', bg:'bg-blue-50' },
 { label:'Опоздания', value: stats.total_shifts > 0 ?`${stats.late_shifts}/${stats.total_shifts}`:'—', icon: ExclamationTriangleIcon, color:'text-yellow-600', bg:'bg-yellow-50' },
 { label:'Пропущ. смены', value: stats.missed_shifts, icon: XCircleIcon, color:'text-red-600', bg:'bg-red-50' },
 { label:'Рейтинг', value:`${stats.performance_score}%`, icon: ChartBarIcon, color: getPerformanceColor(stats.performance_score), bg: stats.performance_score >= 95 ?'bg-green-50' : stats.performance_score >= 85 ?'bg-yellow-50' :'bg-red-50' },
 ].map((stat) => {
 ...
 <div
 className={`h-2 rounded-full ${stats.completion_rate >= 80 ?'bg-green-500' : stats.completion_rate >= 60 ?'bg-yellow-500' :'bg-red-500'}`}
 ...
 <span
 key={type}
 className="inline-flex items-center px-2.5 py-1 rounded-lg bg-gray-50 text-xs text-gray-700 border border-gray-200"
 >
 {responseTypeLabels[type] || type}
 <span className="ml-1.5 font-semibold text-blue-600">{count as number}</span>
 </span>
 ))}
 </div>
 )}
