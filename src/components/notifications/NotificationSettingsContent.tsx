...
export const NotificationSettingsContent: React.FC<NotificationSettingsContentProps> = ({ dealershipId }) => {
 ...
 <Card key={setting.id} className="hover:shadow-sm">
 <Card.Body className="space-y-4">
 {/* Header row with toggle */}
 <div className="flex items-start justify-between gap-4">
 <div className="flex items-start gap-3 flex-1 min-w-0">
 <div className={`flex-shrink-0 mt-0.5 ${setting.is_enabled ?'text-green-500' :'text-gray-400'}`}>
 {setting.is_enabled ? (
 ...
 <h3 className="text-sm font-medium text-gray-900 truncate">{setting.channel_label}</h3>
...
`}
 ...
 <span className="text-xs text-gray-500 hidden sm:inline">Время:</span>
 <Input
 type="time"
 ...
 {setting.channel_type ==='weekly_report' && (
 <div className="flex items-center gap-2">
 <span className="text-xs text-gray-500 hidden sm:inline">День:</span>
 <Select
 ...
 <div className="flex items-center gap-2">
 <ClockIcon className="w-4 h-4 text-gray-400 flex-shrink-0" />
 <span className="text-xs text-gray-500 hidden sm:inline">За:</span>
 <Input
 type="number"
 min={1}
 max={1440}
