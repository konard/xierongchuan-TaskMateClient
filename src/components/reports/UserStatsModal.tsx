...
export const UserStatsModal: React.FC<UserStatsModalProps> = ({
 ...
 <div
 className={`h-3 rounded-full ${employee.completion_rate >= 80 ?'bg-green-500' : employee.completion_rate >= 60 ?'bg-yellow-500' :'bg-red-500'}`}
 ...
 <span
 key={type}
 className="inline-flex items-center px-3 py-1.5 rounded-lg bg-white text-sm text-gray-700 border border-gray-200"
 >
 {responseTypeLabels[type] || type}
 <span className="ml-2 font-semibold text-blue-600">{count}</span>
 </span>
 ))}
 </div>
 </div>
