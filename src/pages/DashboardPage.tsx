...
export const DashboardPage: React.FC = () => {
 ...
 <circle cx="24" cy="24" r="18" fill="none" stroke="currentColor" className="text-gray-200" strokeWidth="5" />
 {ds.total_employees > 0 && (
 <circle
 cx="24" cy="24" r="18" fill="none"
 stroke="currentColor"
 className={ds.on_shift_count > 0 ?'text-green-500' :'text-gray-300'}
 strokeWidth="5"
 ...
 <text x="24" y="24" textAnchor="middle" dominantBaseline="central" className="fill-gray-900" fontSize="11" fontWeight="600">
 {ds.on_shift_count}/{ds.total_employees}
 </text>
 </svg>
 <div className="min-w-0 flex-1">
 <p className="font-semibold text-gray-900 text-sm truncate">{ds.dealership_name}</p>
 ...
 <div key={shift.id} className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-xl bg-white">
 <div className="flex items-center gap-2 min-w-0">
 <span className="relative flex h-2 w-2 flex-shrink-0">
 <span className=" absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
 <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
 </span>
 <span className="text-sm font-medium text-gray-900 truncate">{shift.user?.full_name}</span>
 </div>
 <div className="flex items-center gap-1.5 flex-shrink-0">
 <span className="text-xs text-gray-500">{formatTime(shift.opened_at)}</span>
 {shift.is_late && (
 ...
 className={`flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 p-3 rounded-xl cursor-pointer ${
 task.status ==='overdue'
