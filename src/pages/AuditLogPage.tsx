...
const PayloadViewer: React.FC<{ payload: Record<string, unknown>; action: string }> = ({ payload, action }) => {
 ...
 <h4 className="text-sm font-medium text-gray-700">Изменённые поля:</h4>
 <div className="space-y-2">
 {changedFields.map((field) => (
 <div key={field} className="bg-gray-50 p-3 rounded-xl">
 <div className="text-sm font-medium text-gray-600 mb-2">{field}</div>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <span className="text-xs text-red-600 block mb-1">Было:</span>
 <code className="text-sm bg-red-50 px-2 py-1 rounded text-red-700 block break-all whitespace-pre-wrap">
 {formatPayloadValue(oldData[field])}
 </code>
 </div>
 <div>
 <span className="text-xs text-green-600 block mb-1">Стало:</span>
 <code className="text-sm bg-green-50 px-2 py-1 rounded text-green-700 block break-all whitespace-pre-wrap">
 ...
 return (
 <pre className="bg-gray-100 p-4 rounded-xl overflow-auto max-h-96 text-sm">
...
export const AuditLogPage: React.FC = () => {
 ...
 <span className="text-gray-900 truncate max-w-[150px]" title={log.dealership.name}>
 {log.dealership.name}
 </span>
 </div>
 ) : (
 ...
 <span className="text-gray-900 truncate max-w-[150px]" title={log.actor.full_name}>
 {log.actor.full_name}
 </span>
 </div>
 ) : (
 ...
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="text-sm font-medium text-gray-500">ID записи аудита</label>
 ...
 <p className="text-gray-900">{selectedLog.table_label}</p>
 </div>
 <div>
 ...
 <div>
 <label className="text-sm font-medium text-gray-500">ID записи в таблице</label>
 <p className="text-gray-900 font-mono">#{selectedLog.record_id}</p>
 </div>
 <div>
 <label className="text-sm font-medium text-gray-500">Автосалон</label>
 <p className="text-gray-900">
